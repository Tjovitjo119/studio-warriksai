// ============================================================================
// WARRIKS AI v5.2 — Turtle Breakout Filter Engine Unit Tests
// Tests: 20-period breakout, volatility expansion, volume confirmation
// ============================================================================
...
describe("Turtle Breakout Filter Engine", () => {
  it("returns NEUTRAL with insufficient data", () => {
    ...
    const result = analyzeTurtleBreakout(candles, structure, "NAS100");

    expect(result.type).toBe("TURTLE_BREAKOUT");
  ...
  it("has all required indicator keys", () => {
    ...
    expect(result.indicators).toHaveProperty("breakoutLevel");
    expect(result.indicators).toHaveProperty("breakoutStrength");
    expect(result.indicators).toHaveProperty("volExpansionRatio");
    expect(result.indicators).toHaveProperty("volumeRatio");
    expect(result.indicators).toHaveProperty("htfAligned");
  ...
  it("handles empty data gracefully", () => {
    ...
    const result = analyzeTurtleBreakout([], structure, "NAS100");
    expect(result.direction).toBe("NEUTRAL");
    expect(result.signal).toBe(false);
  });
});
